...
public interface ConferenceParticipantMapper {
    ...
    @Insert("insert into conference_participant(id,conference_id,participant_id,status,create_time,modify_time) values(#{id},#{conferenceId},#{participant_id},#{status},#{createTime},#{modifyTime})")
    int add(ConferenceParticipant conferenceParticipant);

    @Delete("delete from conference_participant where id = #{id}")
    int delete(String id);

    @Update("update conference_participant set conference_id = #{conferenceId} ,participant_id = #{participantId} ,status = #{status} ,modify_time = #{modifyTime} where id = #{id} ")
    ...
    List<String> findParticipant(String participantId);

    @Select("select id,conference_id,participant_id,status from conference_participant where participant_id = #{participantId} and conference_id = #{conferenceId}")
